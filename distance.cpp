...
double get_distance(double x, double y, double z)
{
    ...
    for (int i = 0; i < model.m_all_faces.size(); i++)
    {
        ...
        if (results[0] < 0 && results[4] > 0 && results[6] < 0) dist_temp = sqrt(pow(get_distance_point_line_2D(xyz_new, model.m_all_faces[i].m_point0_new, model.m_all_faces[i].m_point2_new), 2) + pow(xyz_new.m_x, 2));
        if (results[1] < 0 && results[5] > 0 && results[8] < 0) dist_temp = sqrt(pow(get_distance_point_line_2D(xyz_new, model.m_all_faces[i].m_point2_new, model.m_all_faces[i].m_point1_new), 2) + pow(xyz_new.m_x, 2));
        if (results[2] < 0 && results[7] > 0 && results[3] < 0) dist_temp = sqrt(pow(get_distance_point_line_2D(xyz_new, model.m_all_faces[i].m_point1_new, model.m_all_faces[i].m_point0_new), 2) + pow(xyz_new.m_x, 2));
        if (results[4] < 0 && results[3] > 0) dist_temp = get_distance_point_point(model.m_all_faces[i].m_point0_new, xyz_new);
        if (results[5] < 0 && results[6] > 0) dist_temp = get_distance_point_point(model.m_all_faces[i].m_point2_new, xyz_new);
        if (results[7] < 0 && results[8] > 0) dist_temp = get_distance_point_point(model.m_all_faces[i].m_point1_new, xyz_new);
